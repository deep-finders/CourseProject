...
def avg_precision(docs,num_relevant_docs):
    vals_to_avg = [precision(docs[:i+1]) for (i, doc) in enumerate(docs) if doc == 1]

    return sum(vals_to_avg) / num_relevant_docs if docs else 0
    ...
    for result in test_set:
        ...
        for result in results:
            if result["feedback"]=="1":
                result_list.append(result["passage"])
        ...
        try:
            ...
            for i in range(0,max_idx):
                if new_results[i]["passage"] in result_list:
                    found_list.append(1)
                else:
                    found_list.append(0)
            
            ap_counter = ap_counter + avg_precision(found_list,top_n)
            count = count+1
        except:
